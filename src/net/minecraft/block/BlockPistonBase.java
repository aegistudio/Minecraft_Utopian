...
public class BlockPistonBase extends Block implements IBlockPistonBase
{
    ...
    public boolean onBlockEventReceived(World par1World, int par2, int par3, int par4, int par5, int par6)
    {
        ...
        else if (par5 == 1)
        {
            ...
            if (this.isSticky)
            {
                ...
                if (!var13 && var11 > 0 && canPushBlock(var11, par1World, var8, var9, var10, false) && (BlockInfoContainer.getBlockInfoContainer().getBlock(var11).getMobilityFlag() == 0 || var11 == Block.pistonBase.blockID || var11 == Block.pistonStickyBase.blockID))
                {
    ...
    private static boolean canPushBlock(int par0, World par1World, int par2, int par3, int par4, boolean par5)
    {
        ...
        else
        {
            ...
            {
                if (BlockInfoContainer.getBlockInfoContainer().getBlock(par0).getBlockHardness(par1World, par2, par3, par4) == -1.0F)
                {
                    return false;
                }

                if (BlockInfoContainer.getBlockInfoContainer().getBlock(par0).getMobilityFlag() == 2)
                {
                    return false;
                }

                if (BlockInfoContainer.getBlockInfoContainer().getBlock(par0).getMobilityFlag() == 1)
                {
                    if (!par5)
                    {
                        return false;
            ...
            return !(BlockInfoContainer.getBlockInfoContainer().getBlock(par0) instanceof ITileEntityProvider);
        }
    }
        ...
        while (true)
        {
            if (var8 < 13)
            {
                ...
                if (var9 != 0)
                {
                    ...
                    if (BlockInfoContainer.getBlockInfoContainer().getBlock(var9).getMobilityFlag() != 1)
                    {
                        if (var8 == 12)
                        {
                            return false;
        ...
        while (true)
        {
            ...
            if (var9 < 13)
            {
                ...
                if (var10 != 0)
                {
                    ...
                    if (BlockInfoContainer.getBlockInfoContainer().getBlock(var10).getMobilityFlag() != 1)
                    {
                        if (var9 == 12)
                        {
                            return false;
                    ...
                    BlockInfoContainer.getBlockInfoContainer().getBlock(var10).dropBlockAsItem(par1World, var6, var7, var8, par1World.getBlockMetadata(var6, var7, var8), 0);
